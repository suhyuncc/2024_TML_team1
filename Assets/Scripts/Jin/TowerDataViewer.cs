...
public class TowerDataViewer : MonoBehaviour
{
	...
	private void UpdateTowerData()
	{
		...
		if (currentTower.WeaponType == WeaponType.Cannon || currentTower.WeaponType == WeaponType.RangeAttack)
		{
			...
			textDamage.text = "데미지 : " + currentTower.Damage;
		}
		else
		{
			imageTower.rectTransform.sizeDelta = new Vector2(100, 120);
			...
			{
				textDamage.text = "이동속도 감소 : " + currentTower.Slow * 100 + "%";
			}
			else if (currentTower.WeaponType == WeaponType.Buff)
			{
				textDamage.text = "공격속도 증가 : " + currentTower.Buff * 100 + "%";
			}
			else if (currentTower.WeaponType == WeaponType.Debuff)
			{
				textDamage.text = "받는피해 증가 : " + currentTower.Debuff * 100 + "%";
			}
            else if (currentTower.WeaponType == WeaponType.Laser)
            {
                textDamage.text = "데미지 : " + currentTower.Debuff + "~" + currentTower.damageIncreaseRate;
            }
        }
		...
		textLevel.text = "등급 : " + currentTower.Level;
